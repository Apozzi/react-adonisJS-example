    ...
    <div className="min-h-screen bg-[#080a0f] flex items-center justify-center relative overflow-hidden">
      ...
      <div className="relative w-full max-w-sm px-4">
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-[#c8f542] mb-5 shadow-[0_0_40px_rgba(200,245,66,0.3)]">
            <DollarSign size={28} className="text-black" />
          </div>
          <h1 className="text-white text-3xl font-bold tracking-tight">Comissões</h1>
          <p className="text-gray-500 text-sm mt-1">Sistema de gestão de comissionamento</p>
        </div>
        <div className="bg-[#0f1117] border border-white/10 rounded-2xl p-8 shadow-2xl">
          <div className="flex flex-col gap-4">
            <Field label="E-mail">
              <input className={inputCls} type="email" placeholder="seu@email.com" value={email}
                onChange={(e) => { setEmail(e.target.value); setErr('') }}
                onKeyDown={(e) => e.key === 'Enter' && submit()} />
            </Field>
            <Field label="Senha">
              <div className="relative">
                <input className={`${inputCls} w-full pr-10`} type={show ? 'text' : 'password'} placeholder="••••••••"
            ...
            {err && <p className="text-red-400 text-xs flex items-center gap-1"><AlertCircle size={12} />{err}</p>}
            <button onClick={submit} disabled={loading}
              className="mt-2 w-full bg-[#c8f542] text-black font-bold py-3 rounded-xl hover:bg-[#d4f75e] transition-all disabled:opacity-60 shadow-[0_0_20px_rgba(200,245,66,0.2)]">
